...
export class SampleDatePickerNormal implements OnInit {
    ...
    onDateChanged(event: IMyDateModel) {
        ...
        if(event.formatted !== '') {
            this.selectedTextNormal = 'Formatted: ' +this.myDatePickerNormalOptions.monthLabels[event.date.month]+' '+event.date.day+', '+(event.date.year+543) + ' - epoc timestamp: ' + event.epoc;
    ...
    onInputFieldChanged(event: IMyInputFieldChanged) {
        console.log('onInputFieldChanged(): Value: ', event.dateFormat, ' - dateFormat: ', event.dateFormat, ' - valid: ', event.valid);
    }
